...
from src.models import Citation, SearchResult, Source
...
class TestResponseParser:
    ...
    def test_parse_response_extracts_text(self, valid_api_response, sample_query):
        ...
        expected_text = (
            "Recent technology news highlights several exciting developments"
        )
    ...
    def test_parse_response_extracts_sources(self, valid_api_response, sample_query):
        ...
        assert (
            result.sources[0].url == "https://techcrunch.com/2025/10/10/ai-breakthrough"
        )
        assert result.sources[0].type == "web"

    def test_parse_response_with_no_citations(
        self, no_citations_response, sample_query
    ):
    ...
    def test_format_display_with_no_citations(
        self, no_citations_response, sample_query
    ):
