...
class TestSearchService:
    ...
    def test_search_with_options(
        self, mock_client_class, test_api_key, sample_query, sample_allowed_domains
    ):
        ...
        service = SearchService(api_key=test_api_key)
        options = SearchOptions(model="gpt-5", allowed_domains=sample_allowed_domains)
    ...
    def test_search_with_invalid_query_raises_error(
        self, mock_client_class, test_api_key
    ):
    ...
    def test_search_handles_client_error(
        ...
        mock_client.search.side_effect = SearchError(
            code="API_ERROR", message="API request failed"
        )
        mock_client_class.return_value = mock_client
    ...
    @patch("src.search_service.WebSearchClient")
    @patch("src.search_service.ResponseParser")
    def test_search_handles_parser_error(
        self,
        mock_parser_class,
        mock_client_class,
        test_api_key,
